...
| # | 항목 | 결정 |
|---|---|---|
| Q1 | 탄약 규칙 | **원작대로**: 기본 5(1~99), 명중 +1 회복, 빗나감 −1, 이동 중 대상은 무료 |
| Q2 | 점수 | **Missed Spot 도입**, 생존 보너스 축소 |
| Q3 | 순서 | R1 → R2 → R4 (클론 R3은 보류) |
| Q4 | 체형 크기 | 스케일 + 비율로 시작 |
| Q5 | 벽 붙기 키 | **원작대로 E 위 / Q 아래 / Space 떼기**. 모바일은 거의 안 쓰지만 버튼 매핑은 유지·최적화 |
| Q6 | 클론·Double | **클론 시스템은 아직 개발하지 않음** → R3·R5-T1 보류 |
