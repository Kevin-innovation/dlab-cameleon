...
function extraCover(map: GameMap): BoxDef[] {
  if (map.id === "backrooms") {
    ...
    return [
      ...
      // 복도 B
      cabinet(13, 18.4),
      // 문서고: 등을 맞댄 서가 두 줄(양면 서가)과 남쪽 벽의 캐비닛
      shelf(2, 24),
      shelf(5, 24),
      shelf(9, 24),
      shelf(12, 24),
      ...
      cabinet(2, 31.2),
      crate(13.5, 30.2),
      // 라운지: 소파는 북쪽·서쪽 벽에 붙인다
      sofa(2, 32.4, "#7b3f2a"),
      B(-0.65, 37.4, 3.4, 1.25, "#7b3f2a", { h: 0.95, collide: true, prop: "sofa", collider: { w: 3.2, d: 1.12 }, rotation: Math.PI / 2 }),
      armchair(8, 35),
      table(9.5, 37.5),
      plant(14.2, 40),
      lamp(14.5, 33),
      ...
      chair(47.5, 7),
      chair(50, 7),
      // 창고: 선반은 북쪽·서쪽 벽에 붙이고 상자는 안쪽에
      rack(42, 12.4),
      rack(49.5, 12.4),
      B(38.45, 16.7, 4, 0.6, "#7a7f86", { h: 2.2, collide: true, prop: "bookshelf", collider: { w: 3.8, d: 0.54 }, rotation: Math.PI / 2 }),
      B(38.45, 21.2, 4, 0.6, "#7a7f86", { h: 2.2, collide: true, prop: "bookshelf", collider: { w: 3.8, d: 0.54 }, rotation: Math.PI / 2 }),
      crate(52, 21),
  ...
  if (map.id === "mansion") {
    ...
    return [
      ...
      // 주방
      B(0.5, 18.5, 5, 0.7, "#e8e8e2", { h: 0.92, collide: true }),
  ...
  if (map.id === "sewer") {
    ...
    return [
      ...
      crate(34, 14.8, 1.1, 0.9),
      // 수로 통로: 물과 연석, 이끼 벽. 소파는 북쪽 벽에 붙인다.
      { x: 12, y: 0.02, z: 32, w: 24, h: 0.04, d: 6, color: "#2f5d5a", collide: false, role: "decal" as const },
      curb(0.5, 28.6, 23, 0.4),
      B(18, 22.6, 2.2, 1.2, "#365b78", { h: 1.1, collide: true, pattern: "graffiti", colors: ["#365b78", "#f1c40f"] }),
      B(2.5, 20.4, 3.6, 1.3, "#6a3d4b", { h: 0.95, collide: true, prop: "sofa", collider: { w: 3.38, d: 1.16 } }),
      barrel(14, 21, "#922b21"),
